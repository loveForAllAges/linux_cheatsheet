...
## Process monitoring commands
- __ps__: Сообщает о текущих процессах. [Примеры ниже](#ps-examples)
...
## Networking tools
- __traceroute__: Инструмент, который отображает пути, пройденные пакетами по IP-сети, для выявления проблем маршрутизации, задержки сети и потери пакетов. [Примеры ниже](#traceroute-examples)
- `ping`: Инструмент для проверки доступности сетевого хоста, который отправляет ICMP пакеты на хост и ожидает ICMP ответ.
...
- __tcpdump__: Инструмент для перехвата трафика по критериям, мониторинга сетевой активности, записи пакетов в файл для дальнейшего изучения. [Примеры ниже](#tcpdump-examples)
